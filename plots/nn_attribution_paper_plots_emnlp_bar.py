...
def plot_utility_tradeoff(
    ...
    """Plot paired bars for DD@15 and utility, where utility = DD@15 - boundary FPR."""
    set_paper_style()
    data = load_json(utility_json)
    util = data["utility_boundary"]
    methods = [m for m in methods if m in util]

    fig, ax = plt.subplots(figsize=(4.75, 2.45))

    x = np.arange(len(methods))
    width = 0.34

    dd_vals = [float(util[m]["dd"]) for m in methods]
    utility_vals = [
        float(util[m].get("utility", float(util[m]["dd"]) - float(util[m]["fpr"])))
        for m in methods
    ]

    # ERASER-style grouped bars: clean blue/orange series colors, no hatching.
    dd_color = "#8faadc"
    utility_color = "#f4b183"
    ...
    annotate_bars(dd_bars)
    annotate_bars(utility_bars)

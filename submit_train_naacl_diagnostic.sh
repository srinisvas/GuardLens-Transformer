...
# With --export=ALL, Slurm gives inherited exported variables precedence over
# explicit NAME=value entries. Remove every per-job selector from the caller's
# environment so it cannot override the controlled matrix settings below.
...
for index in "${!names[@]}"; do
  ...
  diagnostic_job=$(sbatch --parsable \
    ...
    --export="ALL,CONDA_ENV=$CONDA_ENV,EVAL_DATA=$PREPARED_DEV,EVAL_CHECKPOINT=$MATRIX_ROOT/training/$name/checkpoints/best.pt,EVAL_OUTPUT=$diagnostic_output,EVAL_PROTOCOL=${protocols[$index]},EVAL_SHARDS=$EVAL_SHARDS,MAX_REQUEUES=$REQUEUE_LIMIT" \
    eval_internal_dev.slurm)
